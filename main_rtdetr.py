...
def main():
    ...
    # Автоматический выбор вычислительного устройства
    ...
    # Инициализируем трансформерную модель через отдельный файл архитектуры
    model = get_rtdetr_model("rtdetr-l.pt")
    
    # Формируем имя папки результатов
    experiment_name = f"rtdetr_lr_{args.lr}_batch_{args.batch}"
    
    # Запуск честного обучения по нашему датасету KITTI
    # Используем AdamW (автоматически выставляется движком для DETR-моделей)
    model.train(
        data="configs/default.yaml",   # Наш рабочий конфигурационный файл
        epochs=args.epochs,
        imgsz=320,                     # Стандартное разрешение
        device=current_device,
        project="results",             # Общая директория для всех моделей
        name=experiment_name,
        lr0=args.lr,
        batch=args.batch,
        optimizer="AdamW"              # Принудительно задаем AdamW согласно спецификации отчета
    )
